...
// 기존 데이터와 비교하여 변경된 부분만 업데이트
const updateGameUI = (game, changes) => {
    // Broadcast 텍스트 업데이트
    const broadcastTextElement = document.querySelector('.broadcast-text-area .text');
    if (broadcastTextElement) { // 요소가 존재하는지 확인
        if (changes.broadcast) {
            broadcastTextElement.classList.add('changed-text');
            broadcastTextElement.textContent = game.broadcast ? game.broadcast.playText : '';
        } else {
            broadcastTextElement.classList.remove('changed-text');
        }
    }

    // Base 상태 업데이트
    ['firstBase', 'secondBase', 'thirdBase'].forEach((base, idx) => {
        const baseElement = document.querySelector(`.base.base${idx + 1}`);
        if (baseElement) { // 요소가 존재하는지 확인
            if (changes[`${base}Occupied`]) {
                baseElement.classList.add('changed-base');
                setTimeout(() => baseElement.classList.remove('changed-base'), 500);
            }
            baseElement.classList.toggle('active', game[`${base}Occupied`]);
        }
    });

    // Ball, Strike, Out 업데이트
    ['ball', 'strike', 'out'].forEach((type) => {
        const countElement = document.querySelector(`.${type}`);
        if (countElement) { // 요소가 존재하는지 확인
            [...Array(type === 'ball' ? 4 : 3)].forEach((_, i) => {
                const circleElement = countElement.querySelector(`.${type}-outline-circle:nth-child(${i + 2})`);
                if (circleElement) { // 요소가 존재하는지 확인
                    circleElement.classList.toggle('active', i < game[type]);
                    if (changes[type] && i === game[type] - 1) {
                        circleElement.classList.add(`changed-${type}`);
                        setTimeout(() => circleElement.classList.remove(`changed-${type}`), 500);
                    }
                }
            });
        }
    ...
    // Pitcher와 Batter 업데이트
    const pitcherElement = document.querySelector('.player.pitcher');
    if (pitcherElement) { // 요소가 존재하는지 확인
        pitcherElement.textContent = game.currentPitcher ? game.currentPitcher.name : ' - ';
    }

    const batterElement = document.querySelector('.player.batter');
    if (batterElement) { // 요소가 존재하는지 확인
        batterElement.textContent = game.currentBatter ? game.currentBatter.name : ' - ';
    }
};
...
async function getGameData() {
    ...
    try {
        ...
        try {
            additionalDataResponse = await axios.get(`${SUB_URL}/v1.0/sports/baseball/games?date=${requestDate}&status=ALL`,{
                timeout: 5000
            })
        } catch (error) {
            console.log("데이터를 못 받아왔습니다.")
            additionalDataResponse = null;
            additionalDataYn = false;
        }

        const additionalData = additionalDataResponse ? additionalDataResponse.data : null;

        const mergeData = res.data.map((item) => {
            const additionalItem = additionalData ? additionalData.find(addItem => addItem.id === item.namedGameId) : null;

            if (additionalItem) {
                // 필요한 특정 프로퍼티만 가져옴
                console.log("데이터를 받아옴", additionalData)
                additionalDataYn = true;

                return {
                    ...item,
                    broadcast: additionalItem.broadcast,
                    currentBatter: additionalItem.currentBatter,
                    currentPitcher: additionalItem.currentPitcher,
                    firstBaseOccupied: additionalItem.firstBaseOccupied,
                    secondBaseOccupied: additionalItem.secondBaseOccupied,
                    thirdBaseOccupied: additionalItem.thirdBaseOccupied,
                    ball: additionalItem.ball,
                    out: additionalItem.out,
                    strike: additionalItem.strike,
                };
            } else {
                // 추가 데이터가 없으면 원본 데이터만 반환
                return item;
            }
        })

        const gameInfo = countEntries(mergeData);
