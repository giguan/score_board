...
async function getGameData() {
    ...
    const basketballCategory = ["국제", "USA", 'Philippines']

    //야구의 경우 미국 대한민국 일본이 카테고리임
    const res = await axios.get('http://localhost:3000/proxy/basketball/match-list', {
        params: {
            date: requestDate
        }
    })
    .then(response => {
        ...
        const filteredData = response.data.filter(match => basketballCategory.includes(match.category_name));
